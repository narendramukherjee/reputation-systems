...
def posterior_pair_plot_per_product(
    posterior_samples: np.ndarray, products_to_plot: List, simulator: marketplace_simulator_class.MarketplaceSimulator
) -> None:
    parameters = [
        r"$\rho_{-}$",
        r"$\rho_{+}$",
    ...
    max_bias_5_star = simulator.max_bias_5_star
    one_star_lowest_limit = simulator.one_star_lowest_limit
    five_star_highest_limit = simulator.five_star_highest_limit
    for i, product in enumerate(products_to_plot):
        df = pd.DataFrame(posterior_samples[:, product, :])
        df.columns = parameters
        df[r"$p_2$"] *= one_star_lowest_limit * df[r"$p_1$"]
        df[r"$p_1$"] *= one_star_lowest_limit
        df[r"$p_4$"] *= five_star_highest_limit * df[r"$p_5$"]
        df[r"$p_5$"] *= five_star_highest_limit
        df[r"$bias_{5 star}$"] *= max_bias_5_star
        df["chain"] = 0
        df["draw"] = np.arange(len(df), dtype=int)
        df = df.set_index(["chain", "draw"])
        xdata = xr.Dataset.from_dataframe(df)
        dataset = az.InferenceData(posterior=xdata)
        az.plot_pair(dataset, kind="kde", marginals=True)


def forest_plot_per_parameter(
    posterior_samples: np.ndarray, simulator: marketplace_simulator_class.MarketplaceSimulator
) -> None:
    parameters = [
        r"$\rho_{-}$",
        r"$\rho_{+}$",
    ...
    for i, parameter in enumerate(parameters):
        if i < 3:  # for rho-, + , and h_p
            multiplier = 1
        elif i == 3:  # for p_1
            multiplier = one_star_lowest_limit
        elif i == 4:  # for p_2
            multiplier = one_star_lowest_limit * posterior_samples[:, :, 3].copy()
        elif i == 5:  # for p_4
            multiplier = five_star_highest_limit * posterior_samples[:, :, 6].copy()
        elif i == 6:  # for p_5
            multiplier = five_star_highest_limit
        else:  # for bias_5star
            multiplier = max_bias_5_star
        az.plot_forest({parameter: (multiplier * posterior_samples[:, :, i]).T}, linewidth=0.5, markersize=0.5)
...
def plot_simulated_vs_actual_total_reviews(simulations: np.ndarray, observations: pd.DataFrame) -> None:
    ...
    plt.hist(
        obs_total_num_reviews, color=sns.xkcd_rgb["dark orange"], alpha=0.5, label="Actual", density=True, bins=bins
    )
...
def main() -> None:
    ...
    # Plot the posterior pairplots for a handful of products
    posterior_pair_plot_per_product(posterior_samples, [2, 10, 1400, 956], simulator)
    # Make forest plots (1 for each parameter) across all the products
    forest_plot_per_parameter(posterior_samples, simulator)
    # Plot the distribution of the mean of the posteriors across products
    plot_mean_posteriors_for_products(posterior_samples)

    # Use the simulations to plot a few to show the variety of histograms that can be generated in the simulations
