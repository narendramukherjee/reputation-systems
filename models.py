...
class consumer(product):
    ...
    def make_purchase(self):
        expected_utilities = list(self.consumer_private_beta * np.array(self.params['product_features']) + self.consumer_private_alpha * np.array(self.params['prices']) +
    ...
    def decide_to_rate(self,product_index,product_review):
        ...
        if np.random.binomial(1, self.params['tendency_to_rate']):
            ...
            decision = (abs(product_review - self.avg_reviews[product_index][-1]) > 0.5) \
...
class market(consumer):
    ...
    def step(self):
        ...
        if self.decide_to_rate(product_index,product_review):
            ...
            return product_index,product_review
        else:
    ...
    def generateTimeseries(self):  # conditioned on the fixed_params
        ...
        while (len(timeseries) < self.params['total_number_of_reviews']):
            product_index, product_review = self.step()
            if product_review is not None:
                timeseries.append((product_index, product_review))
    ...
    def genTorchSample(self):
        ...
        data = torch.FloatTensor(df.values[:, 0:2])
        label_of_data = torch.LongTensor([self.params['population_alpha'],self.params['population_beta'],self.params['true_qualities']])
